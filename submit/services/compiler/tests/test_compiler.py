...
class TestGetTaskStatus(TestCase):
    """Tests for :mod:`compiler.get_task_status` with mocked responses."""

    @mock.patch(f'{compiler.__name__}.requests.Session')
    def test_get_task_status_failed(self, mock_Session):
        ...
        mock_Session.return_value = mock_session
        comp_status = compiler.get_task_status(upload_id, task_id)
        self.assertEqual(comp_status.upload_id, upload_id)
        self.assertEqual(comp_status.task_id, task_id)
        self.assertEqual(comp_status.status,
    ...
    @mock.patch(f'{compiler.__name__}.requests.Session')
    def test_get_task_status_in_progress(self, mock_Session):
    ...
    @mock.patch(f'{compiler.__name__}.requests.Session')
    def test_get_task_status_completed(self, mock_Session):
    ...
    @mock.patch(f'{compiler.__name__}.requests.Session')
    def test_get_task_status_doesnt_exist(self, mock_Session):
        ...
        with self.assertRaises(compiler.NoSuchResource):
            compiler.get_task_status(upload_id, task_id)
